...
function WebGLSetup(id) {
  ...
  function onUpdate() {
    // Translate box
    cube.position.y = Math.cos(performance.now() * 0.0005) * 1
    ...
    // Scale box
    const scale = 1 + Math.abs(Math.cos(performance.now() * 0.0025) * 0.25)
...
function CanvasSetup(id) {
  ...
  // Create a box
  const geometry = new THREE.BoxGeometry( 1, 1, 1 )
  const material = new THREE.MeshBasicMaterial({ vertexColors: THREE.FaceColors, overdraw: 1 })
  const cube     = new THREE.Mesh( geometry, material )
